...
STRICT INSTRUCTIONS:
1. DEDUPLICATION: If multiple sources (e.g., Globes and TheMarker, or Reuters and Bloomberg) report on the exact same event, compile them into ONE single item. Do not repeat the same event in different cards.
2. DEEP LINKS REQUIRED: For each item, specify a 'sources' array of objects. Each object must contain 'name' (the source name) and 'url'. The 'url' MUST be the exact, deep, direct URL to the specific news article on that website. DO NOT output home domains (like globes.co.il or bloomberg.com) under any circumstances. Extract the exact link from your search grounding results.
3. Hebrew Language: All text fields (title, summary, implication_global, implication_local, category) must be written in high-quality, professional financial Hebrew.
4. Format: Respond strictly with a JSON array conforming to this schema:
[{
    "id": "unique_string",
    "title": "Main news headline in Hebrew",
    "summary": "Clear, concise summary of the article in Hebrew",
...
def fetch_real_news_from_gemini():
    ...
    # ניסיונות חוזרים (Backoff) במקרה של שגיאת תקשורת
...
def generate_html(news_data):
    ...
    # המרת נתוני החדשות למבנה JavaScript
...
<body class="text-slate-100 min-h-screen flex flex-col selection:bg-blue-500/30">
    ...
    <main class="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-8">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-between items-center bg-slate-900/20 border border-slate-800/40 p-4 rounded-2xl">
            <div class="relative w-full sm:w-72">
                ...
                <input type="text" id="searchInput" oninput="filterNews()" placeholder="חיפוש חופשי בכותרות ובתקצירים..." class="w-full bg-slate-950/60 border border-slate-800/60 rounded-xl py-2 pl-4 pr-9 text-xs focus:outline-none focus:border-blue-500 text-slate-200">
            </div>
    ...
    <footer class="border-t border-slate-900 bg-slate-950/40 py-6 text-center text-xs text-slate-500">
        <p>מערכת הבוקר הכלכלי © {datetime.now().year}. הופק אוטומטית באמצעות Gemini 2.5 Pro.</p>
    </footer>
    ...
    <script>
        ...
        function renderNews(newsArray) {{
            ...
            newsArray.forEach(item => {{
                const badgeColor = item.importance === 'High' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20';
                ...
                const sourcesHtml = item.sources.map(src => {{
                    return `
                        <a href="\${src.url}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 bg-slate-850 hover:bg-slate-800 text-slate-300 border border-slate-700/50 text-[10px] px-2.5 py-0.5 rounded-md transition-all">
                ...
                const primaryLink = item.sources[0]?.url || "#";
                ...
                card.innerHTML = `
                    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/40 pb-3">
                        <div class="flex flex-wrap items-center gap-2">
                            \${scopeBadge}
                            <span class="text-[10px] px-2 py-0.5 rounded-lg font-bold \${badgeColor}">\${item.importance === 'High' ? 'חשיבות עליונה' : 'בינונית'}</span>
                            <span class="text-[10px] text-slate-500 font-semibold bg-slate-800/20 px-2 py-0.5 rounded-lg">\${item.category}</span>
        ...
        function filterNews() {
            ...
            const filtered = newsData.filter(item => {
                ...
                const matchesScope = scopeVal === "all" || item.scope === scopeVal;
                const matchesImportance = importanceVal === "all" || item.importance === importanceVal;
...
if __name__ == "__main__":
    if GEMINI_API_KEY == "YOUR_GEMINI_API_KEY_HERE":
        print("אנא הגדר את מפתח ה-Gemini API שלך בסקריפט או כמשתנה סביבה (Environment Variable).")
    else:
